...
**Flag before Nigeria content goes to Ida for sign-off:** RD 1155/2024 (effective May 2025) is a substantial update to Spanish immigration regulation. Don't assume the existing Nigeria source material already accounts for it — worth an explicit check against the current text before treating that content as final, not just a routine legal read-through.
...
- **Family members (SLF) — architecture corrected: document types are global, not nationality-specific.** The Nigeria source material separates individual (SLU) requirements from accompanying family member (SLF) requirements, but SLF was initially treated as if it needed rebuilding per nationality like SLU content does. It doesn't: across Spain's own consulate guidance and independent legal sources, the SLF document *types* (marriage certificate for a spouse, birth certificate for a child, notarized guardian authorization for a minor traveling with one parent, criminal record check for family 18+, medical certificate) are set nationally by Spanish immigration law (Article 53.b), identical regardless of the applicant's nationality — same tier as the IPREM funds formula, not a per-country overlay. What genuinely varies isn't the document list, it's the *legalization path* for each document — and the real decision variable is narrower than "region/continent": whether the issuing country is a Hague Apostille Convention signatory. Hague members (most countries) get a simple apostille; non-members (Nigeria's case) need the fuller consular legalization chain. This is now modeled as a `country_profiles` table (`nationality`, `is_hague_apostille_signatory`, `official_slf_source_link`), separate from the per-document `requirements` rows, which can now have a null `nationality` to mean "applies to every nationality" (SLF rows only — SLU rows still require a real nationality). Adding SLF support for a second country is answering one yes/no Hague-status question, not rebuilding the document list. Content itself (which countries are Hague signatories, the actual non-Hague legalization chain text per country) still needs to be researched/authored and sign-off gated the same as everything else in these tables — the schema exists, the content doesn't yet.
...
Not every row is nationality-specific, though — the table has two real tiers, and it's worth being explicit about which fields sit in which. **Global rules** apply regardless of nationality: the IPREM funds formula shape (100% + 75% first dependent + 50% each additional) and, per the SLF correction above, the SLF document *types* themselves (a `nationality = null` row means "applies to every nationality"). **Country-specific overlays** are the genuinely per-nationality facts: the actual legalization chain wording, `home_country_apostille_authority`, and — new — a country's Hague Apostille Convention membership (`country_profiles.is_hague_apostille_signatory`), which is the single variable that actually decides an SLF document's legalization path (simple apostille vs. full consular chain), not a per-document field to re-derive each time.
